...
const BikesMain = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const bikesRef = firestore().collection("Shop by Brand");

                const [bikesSnapshot] = await Promise.all([
                    bikesRef.get(),
                    
                ]);

                const bikesItems = bikesSnapshot.docs.map(doc => ({ id: doc.id, text: doc.data().Name, imageUrl: doc.data().Image }));

                setBikesItems(bikesItems);
                setLoading(false);
            } catch (error) {
                setError(error);
                setLoading(false);
