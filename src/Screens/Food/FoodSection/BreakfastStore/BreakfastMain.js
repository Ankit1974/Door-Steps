...
const BreakfastMain = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const breakfastItems = breakfastSnapshot.docs.map(doc => ({
                    id: doc.id,
                    text: doc.data().Name,
    ...
    const handleProductPress = (product) => {
        if (product.text === "Tata Tea Gold Aromatic Black Tea Pouch (500g)") {
            navigation.navigate('Product1');
        } else {
            // Handle navigation for other products here
        }
    };
