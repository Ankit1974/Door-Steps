...
const ChocolatesMain = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const chocolatesRef = firestore().collection("Chocolates");
                const chocolatesSnapshot = await chocolatesRef.get();
                const chocolatesItems = chocolatesSnapshot.docs.map(doc => ({
                    id: doc.id,
                    text: doc.data().Name,
                ...
                setChocolatesItems(chocolatesItems);
                setLoading(false);
            } catch (error) {
                setError(error);
                setLoading(false);
