...
const CookingEssentialsMain = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const cookingtRef = firestore().collection("Cooking Essentials");
                const cookingtSnapshot = await cookingtRef.get();
                const cookingtItems = cookingtSnapshot.docs.map(doc => ({
                    id: doc.id,
                    text: doc.data().Name,
...
export default CookingEssentialsMain;
