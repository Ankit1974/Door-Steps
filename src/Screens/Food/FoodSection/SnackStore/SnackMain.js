...
const SnackMain = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const snacksRef = firestore().collection("Snacks Cornors");
                const snacksSnapshot = await snacksRef.get();
                const snacksItems = snacksSnapshot.docs.map(doc => ({
                    id: doc.id,
                    text: doc.data().Name,
