...
const VitaminMain = ({ navigation }) => {
    const [VitaminItems, setVitaminItems] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const vitaminRef = firestore().collection("Vitamin Supplements");
                const vitaminSnapshot = await vitaminRef.get();
                const vitaminItems = vitaminSnapshot.docs.map(doc => ({
                    id: doc.id,
                    text: doc.data().Name,
