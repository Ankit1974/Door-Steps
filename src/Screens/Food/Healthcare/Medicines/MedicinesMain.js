...
const Medicines1Main = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const medicinesRef = firestore().collection("Medicines Supplies");
                const medicinesSnapshot = await medicinesRef.get();
                const medicinesItems = medicinesSnapshot.docs.map(doc => ({
                    id: doc.id,
                    text: doc.data().Name,
