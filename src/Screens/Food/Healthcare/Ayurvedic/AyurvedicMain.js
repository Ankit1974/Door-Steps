...
const AyurvedicMain = ({ navigation }) => {
    const [AyurvedicItems, setAyurvedicItems] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const ayurvedicRef = firestore().collection("Ayurvedic Supplements");
                const ayurvedicSnapshot = await ayurvedicRef.get();
                const ayurvedicItems = ayurvedicSnapshot.docs.map(doc => ({
                    id: doc.id,
                    text: doc.data().Name,
                ...
                setAyurvedicItems(ayurvedicItems);
                setLoading(false);
            } catch (error) {
                setError(error);
                setLoading(false);
