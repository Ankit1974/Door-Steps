...
const TyresMain = ({ navigation }) => {
    const [TyresItems, setTyresItems] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const tyresRef = firestore().collection("Tyres");
                const tyresSnapshot = await tyresRef.get();
                const tyresItems = tyresSnapshot.docs.map(doc => ({
                    id: doc.id,
                    text: doc.data().Name,
