...
const SafetySecurityMain = ({ navigation }) => {
    const [SafetySecurityItems, setSafetySecurityItems] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const safetySecurityRef = firestore().collection("Safety & Security");
                const safetySecuritySnapshot = await safetySecurityRef.get();
                const safetySecurityItems = safetySecuritySnapshot.docs.map(doc => ({
                    id: doc.id,
                    text: doc.data().Name,
                ...
                setSafetySecurityItems(safetySecurityItems);
                setLoading(false);
            } catch (error) {
                setError(error);
                setLoading(false);
...
export default SafetySecurityMain;
